...
import org.z.data.segment.ModleSegment;
import org.z.data.segment.ModleSegment.mSegTerm;

import com.mongodb.BasicDBList;
import com.mongodb.BasicDBObject;

public final class SmartWordFilter extends TokenFilter {
	protected SmartWordFilter(TokenStream input) {
		super(input);
	}
	...
	protected static Logger logger = LoggerFactory.getLogger(SmartWordFilter.class);
	@SuppressWarnings("unused")
	private int minGram, maxGram;
	private TokenMode mode = null;

	public SmartWordFilter(TokenStream in, int minGram, int maxGram, TokenMode mode) {
		super(in);
		if (minGram < 1) {
	...
	private boolean specialWord(BasicDBObject value) {
		content = value.getString("w");
		Set<mSegTerm> set = null;
		set = ModleSegment.NewSegmentModleAll(content);
