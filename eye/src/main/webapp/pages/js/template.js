...
JsEvalContext.prototype.constructor_ = function(opt_data, opt_parent) {
  ...
  me.vars_[VAR_index]=0;
  me.vars_[VAR_count]=0;
};
...
JsEvalContext.prototype.setGlobalArgs = function(name,value) {
    var args=JsEvalContext.globals_["$args"];
    args[name]=value;
};
...
JsEvalContext.setGlobal("$args",{"index":0,"childIndex":0});
...
function jstProcess(context, template, opt_debugging) {
   ...
   * @type Document
   * bug crash with jquery-ui-10.2.1.js (dialog method)
   * modified by xiong1989win@126.com
   * at 2013年8月23日 21时12分17秒
   */
  processor.document_ = ownerDocumentT(template);
...
var JST_ATTRIBUTES = [
    ...
    [ ATT_html, jsEvalToFunction ],
    [ ATT_call, jsEvalToFunction ],
...
//Jason.Xiao move here,in order to always caculate.  
  ...
  var attIf = jstAttributes[ATT_if];
  if (attIf) {
    var shouldDisplay = context.jsexec(attIf, template);
    if (MAPS_DEBUG && me.debugging_) {
      me.logs_.push(attIf + ': ' + shouldDisplay + '<br/>');
    }
    if (!shouldDisplay) {
      displayNone(template);
      return;
  ...
  // terminate traversal.
  var html_=jstAttributes[ATT_html];
  if (html_){
	  me.jstHtml_(context,template,html_);
	  return;
  }
...
JstProcessor.prototype.jstHtml_ = function(context, template, content) {
	  // NOTE(mesch): Profiling shows that this method costs significant
	  // time. In jstemplate_perf.html, it's about 50%. I tried to replace
	  // by HTML escaping and assignment to innerHTML, but that was even
	  // slower.
