...
var ptool = {
	...
	getData:function(pageNum,pageSize){
		...
		$("#dataList").html("<tr><td width=\"100\">国美SKUID</td><td width=\"300\">京东对应商品的链接</td><td width=\"80\">匹配方式</td><td width=\"150\">匹配时间</td><td width=\"100\">操作</td></tr>");
		if(gCatId.length < 1){
			alert("请选择对应的三级分类！");
			return;
		}
					...
					if(data.msg=="success"){
						$.each(data.result,function(i,item){
							...
							for(var j=0;j<arrId.length;j++){
								links += "http://item.jd.com/"+arrId[j]+".html<br>";
							}
							...
							var htmlItem = "<tr>" +
													...
													"</td>" +
												"</tr>";
							$("#dataList").append(htmlItem);
						});
						$("#pageButtons").remove();
						$("#dataList").after(ctool.pagination(data.pageMsg));
	...
	addInputItem:function(){
		...
		var inputItem = "<tr tag=\"inputItem\"><td><input type=\"text\" id=\"inProdId"+ inputNo +"\" style='height:30px' onblur='ptool.checkIsGome(this)'></td><td><input type=\"text\" id=\"inIsGome"+ inputNo +"\" style='height:30px'></td><td><input type=\"text\" onblur=\"ptool.checkSku(this)\" tag=\"inSkuId"+ inputNo +"\" style='height:30px'></td><td><input type=\"text\" id=\"inName"+ inputNo +"\" style='height:30px'></td><td><input type=\"text\" id=\"inBrand"+ inputNo +"\" style='height:30px'></td><td><textarea id=\"inJdLink"+ inputNo +"\" cols='40' rows='1'></textarea></td></tr>";
