...
public class IPSeeker {
	...
	protected static Logger logger = LoggerFactory.getLogger(IPSeeker.class);
	...
	public synchronized static String getCityNameByIP(String ip) {
		if (StringUtil.isEmpty(ip) || instance == null || ip.equals("unknown")) {
			return "";
		}
		String city = "";
		try {
			city = instance.getAddress(ip).trim();
			String[] cities = city.split("\\ ");
			if (cities.length >= 1) {
				city = cities[0];
			}
			int pos = city.indexOf("省");
			if (pos > 0) {
				city = city.substring(pos + 1, city.length());
			}
			pos = city.indexOf("市");
			if (pos > 0) {
			...
			logger.error("getCityNameByIP, ip = " + ip, e.getMessage());
		}
		return city;
	}

	public static String getCityByIP(String ip) {
		return instance.getAddress(ip);
	}
	...
	public IPSeeker() throws Exception {
		...
		this.ipFile = new RandomAccessFile(fileName, "r");
	...
	public IPLocation getIpLocation(String ip) {
		...
		try {
			...
			logger.error("getIpLocation,ip=" + ip);
		}
		if (ipLocation == null) {
	...
	private String readString(int offset) {
		try {
			...
			logger.error("readString, offset = " + offset, e);
		}
		return "";
	}
	...
	public static void main(String[] args) {
		try {
			IPSeeker.init();
			for(int i = 0; i < 10; i++){
				new MyThread("220.181.108.120").start();
				new MyThread("220.181.89.196").start();
				new MyThread("101.226.168.219").start();
				new MyThread("66.249.74.42").start();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
...
class MyThread extends Thread{
	...
	@Override
	public void run() {
			System.out.println(IPSeeker.getCityNameByIP(this.ip));
	}
}
